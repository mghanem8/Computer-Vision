...
class Prob5():
    def __init__(self):
        ...
        ###### START CODE HERE ######
        self.cat = cv2.imread("cat.bmp")
        self.cat = cv2.cvtColor(self.cat, cv2.COLOR_BGR2RGB)
        self.cat = self.cat.astype(float) / 255
        self.dog = cv2.imread("dog.bmp")
        self.dog = cv2.cvtColor(self.dog, cv2.COLOR_BGR2RGB)
        self.dog = self.dog.astype(float) / 255 
        ###### END CODE HERE ######
    ...
    def my_conv2d(self, image, filter):
        ...
        np.clip(convolved_image, 0, 1, out=convolved_image)
        ###### END CODE HERE ######
    ...
    def create_hybrid_image(self, image1, image2):
        ...
        hybrid_image = None
        ###### START CODE HERE ######
        low_freq = self.my_conv2d(image1, self.gaussian_filter)
        high_freq = image2 - self.my_conv2d(image2, self.gaussian_filter)
